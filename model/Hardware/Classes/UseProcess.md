...
For each `UseProcess` or subclass except the`planProcess` there is at least one `/Core/Relationship` class or subclass with the relationshipType of 'effected’ on the from and a `/Core/Element` class or subclass on the to.
